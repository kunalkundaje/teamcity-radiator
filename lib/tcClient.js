  ...
  fetchBuilds: function (data, callback) {
    async.map(data.stages, 
      function (stage, callback) {
        var requestUrl = data.teamCityUrl + '/builds/?locator=buildType:' + stage.buildTypeId + ',count:10,running:any,canceled:any';
  ...
  fetchChanges: function(data, callback) {
    async.each(data.pipelines, 
      function (pipeline, callback) {
        var requestUrl = data.teamCityUrl + '/changes?locator=build:(id:' + pipeline.id + ')';
        helpers.getResponseAsJSObject(requestUrl, function (err, result) {
            ...
            async.each(changes, 
              function (change, callback) {
                var changeId         = change.$.id,
                    changeRequestUrl = data.teamCityUrl + '/changes/id:' + changeId;
